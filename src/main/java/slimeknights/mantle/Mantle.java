...
public class Mantle {
  ...
  public Mantle(IEventBus bus, net.neoforged.fml.ModContainer modContainer) {
    ...
    bus.addListener(net.neoforged.bus.api.EventPriority.NORMAL, false, net.neoforged.neoforge.network.event.RegisterPayloadHandlersEvent.class, MantleNetwork.INSTANCE::register);
  ...
  private void register(RegisterEvent event) {
    ...
    if (key == Registries.RECIPE_SERIALIZER) {
      ...
      // predicates
      {
        ...
        // register mob types
        // MobTypePredicate.MOB_TYPES.register(ResourceLocation.fromNamespaceAndPath("undefined"), MobType.UNDEFINED);
        // MobTypePredicate.MOB_TYPES.register(ResourceLocation.fromNamespaceAndPath("undead"), MobType.UNDEAD);
        // MobTypePredicate.MOB_TYPES.register(ResourceLocation.fromNamespaceAndPath("arthropod"), MobType.ARTHROPOD);
        // MobTypePredicate.MOB_TYPES.register(ResourceLocation.fromNamespaceAndPath("illager"), MobType.ILLAGER);
        // MobTypePredicate.MOB_TYPES.register(ResourceLocation.fromNamespaceAndPath("water"), MobType.WATER);
      ...
      BlockEntityTypeRegistryAdapter adapter = new BlockEntityTypeRegistryAdapter(net.minecraft.core.registries.BuiltInRegistries.BLOCK_ENTITY_TYPE);
      Set<Block> signs = MantleSignBlockEntity.buildSignBlocks();
      if (!signs.isEmpty()) {
        slimeknights.mantle.registration.MantleRegistrations.SIGN = adapter.register(MantleSignBlockEntity::new, signs, "sign");
      }
      signs = MantleHangingSignBlockEntity.buildSignBlocks();
      if (!signs.isEmpty()) {
        slimeknights.mantle.registration.MantleRegistrations.HANGING_SIGN = adapter.register(MantleHangingSignBlockEntity::new, signs, "hanging_sign");
      }
    }
