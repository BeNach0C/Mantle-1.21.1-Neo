...
public class BlockDeferredRegister extends DeferredRegisterWrapper<Block> {
  ...
  public BlockDeferredRegister(String modID) {
    super(net.neoforged.neoforge.registries.DeferredRegister.createBlocks(modID), modID);
    this.itemRegister = SynchronizedDeferredRegister.create(net.neoforged.neoforge.registries.DeferredRegister.createItems(modID));
  }
  ...
  public WoodBlockObject registerWood(String name, Function<WoodVariant,BlockBehaviour.Properties> behaviorCreator, boolean flammable) {
    ...
    // signs
    DeferredBlock<StandingSignBlock> standingSign = registerNoItem(name + "_sign", () -> new MantleStandingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F), woodType));
    DeferredBlock<WallSignBlock> wallSign = registerNoItem(name + "_wall_sign", () -> new MantleWallSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F).lootFrom(standingSign), woodType));
    DeferredBlock<MantleCeilingHangingSignBlock> hangingSign = registerNoItem(name + "_hanging_sign", () -> new MantleCeilingHangingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F), woodType));
    DeferredBlock<MantleWallHangingSignBlock> wallHangingSign = registerNoItem(name + "_wall_hanging_sign", () -> new MantleWallHangingSignBlock(behaviorCreator.apply(WoodBlockObject.WoodVariant.PLANKS).instrument(NoteBlockInstrument.BASS).forceSolidOn().noCollission().strength(1.0F).lootFrom(hangingSign), woodType));
   ...
   */
  public DeferredBlock<FlowerPotBlock> registerPotted(String name, Supplier<? extends Block> block) {
    DeferredBlock<FlowerPotBlock> potted = registerNoItem("potted_" + name, () -> new FlowerPotBlock(() -> (FlowerPotBlock)Blocks.FLOWER_POT, block, POTTED_PROPS));
    ((FlowerPotBlock)Blocks.FLOWER_POT).addPlant(resource(name), potted);
    return potted;
  }

  /** Registers a potted form of the given block using the vanilla pot */
  public DeferredBlock<FlowerPotBlock> registerPotted(DeferredBlock<? extends Block> block) {
    return registerPotted(block.getId().getPath(), block);
  }

  /** Registers a potted form of the given block using the vanilla pot */
  public DeferredBlock<FlowerPotBlock> registerPotted(ItemObject<? extends Block> block) {
    return registerPotted(block.getId().getPath(), block);
  }
  ...
  public MetalItemObject registerMetal(String name, String tagName, Supplier<Block> blockSupplier, Function<Block,? extends BlockItem> blockItem, Item.Properties itemProps) {
    ...
    net.neoforged.neoforge.registries.DeferredHolder<Item, Item> ingot = itemRegister.register(name + "_ingot", itemSupplier);
    net.neoforged.neoforge.registries.DeferredHolder<Item, Item> nugget = itemRegister.register(name + "_nugget", itemSupplier);
