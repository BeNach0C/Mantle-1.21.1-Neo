...
public class MantleLoot {
  ...
  public static void registerGlobalLootModifiers(final RegisterEvent event) {
    ...
    if (key == net.neoforged.neoforge.registries.NeoForgeRegistries.Keys.GLOBAL_LOOT_MODIFIER_SERIALIZERS) {
      RegistryAdapter<MapCodec<? extends IGlobalLootModifier>> adapter = new RegistryAdapter<>((Registry<MapCodec<? extends IGlobalLootModifier>>) Objects.requireNonNull(event.getRegistry()));
      ...
      // TAG_EMPTY = Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, Mantle.getResource("tag_empty"), new LootItemConditionType(TagEmptyCondition.CODEC)); // TODO: fix this
      // TAG_FILLED = Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, Mantle.getResource("tag_filled"), new LootItemConditionType(TagFilledCondition.CODEC)); // TODO: fix this
